...
describe("sqlite.provider path resolution", () => {
  ...
  it("normalizes backend name safely", () => {
    ...
    expect(normalizeBackend("CUSTOM-ENDPOINT")).toBe("custom-endpoint");
  ...
  it("derives dashboards path from SPARQL_BACKEND by default", () => {
    process.env["SPARQL_BACKEND"] = "custom";
    expect(resolveDashboardsPath()).toBe(path.resolve("./data/custom.sqlite"));
  });
