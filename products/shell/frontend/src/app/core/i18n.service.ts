...
export class I18nService implements OnDestroy {
  ...
  formatNumber(value: number, options?: Intl.NumberFormatOptions): string {
    return new Intl.NumberFormat(this.language() === 'es' ? 'es-AR' : 'en-US', options).format(value);
  }
  formatDate(value: Date | string | number, options?: Intl.DateTimeFormatOptions): string {
    return new Intl.DateTimeFormat(this.language() === 'es' ? 'es-AR' : 'en-US', options).format(new Date(value));
  }
  ngOnDestroy(): void {
    this.unsubscribe();
  }
