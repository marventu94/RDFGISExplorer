...
**Cartel de carga por etapas:** al abrir un tablero (`?dashboardId=…` o el menú "Tableros") el overlay `features/dashboard/load-progress` muestra las etapas reales del pipeline con su duración: *Recuperando el tablero* (`GET /api/dashboards/:id` + layout/filtros), *Ejecutando la consulta* (`POST /api/query/execute`; el detalle final trae filas, `meta.durationMs` del endpoint y si truncó), *Procesando resultados*, *Calculando resumen* y *Renderizando vistas*. El estado vive en `DashboardLoadProgressService` (signals) sobre el modelo puro `shared/progress/load-stages.ts` (reducers inmutables + `formatDuration`); lo maneja `DashboardPersistenceService` y las vistas y el `summary-panel` solo **reportan** (`reportViewRendered`, `reportSummaryStart/Done`) — fuera de una corrida activa esos reportes son no-ops, así la query ejecutada a mano desde el editor no muestra nada. El cartel se cierra cuando pintaron **las vistas del layout** (`expectViews(layout.visibleSlots())`), con un margen de 2,5 s por si alguna no reporta; el resumen del backend, que puede tardar, queda marcado "sigue en segundo plano" en vez de bloquear. Ojo: el fan-out de `SelectionService` es sincrónico, así que `expectViews` se declara **antes** de `setQueryResult`. `DashboardPersistenceService.isHydrating` sigue siendo el flag de datos, no el del cartel.
